...
=head1 SYNOPSIS
    ...
    # Common usage
    LIVR::Validator.default-auto-trim(1);
    ...
    if ( my $valid_data = $validator.validate($user-data) ) {
       save-user($valid_data);
    } else {
    ...
    $validator.register-rules( 'strong_password' =>  sub {
    ...
    my $default_rules = LIVR::Validator.get-default-rules();

    while ( my ($rule_name, $rule_builder) = each %$default_rules ) {
        LIVR::Validator.register-default-rules($rule_name => sub {
...
if $IS-AUTO-TRIM is undef than default-auto-trim value will be used.
...
&RULE_BUILDER - is a subtorutine reference which will be called for building single rule validator.
    ...
    LIVR::Validator.register-default-rules( my_rule => sub {
        my ($arg1, $arg2, $arg3, $rule_builders) =  @_;

        # $rule_builders - are rules from original validator
        # to allow you create new validator with all supported rules
        # my $validator = LIVR::Validator.new(livr-rules => $livr)->register-rules(%$rule_builders)->prepare();
...
Here is "max_number" implemenation:
    ...
    LIVR::Validator.register-default-rules( max_number => \&max_number );
...
=head2 LIVR::Validator.default-auto-trim($IS_AUTO_TRIM)
